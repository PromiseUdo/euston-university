...
// app/faculties/[...slug]/components/lecturer.tsx
import React from "react";
import { Linkedin, Facebook, ArrowRight } from "lucide-react";
import Link from "next/link";
import Image from "next/image";
...
interface LecturerSectionProps {
  lecturers: SpeakerItem[];
}

const LecturerSection: React.FC<LecturerSectionProps> = ({ lecturers }) => {
  return (
    <section className="py-20 bg-white">
      <MaxWidthWrapper>
        <div className="mb-10 text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
          {lecturers.map((item, index) => (
            <div key={index} className="group text-center">
              ...
              <span className="text-base text-gray-600">{item.designation}</span>
            </div>
          ))}
        </div>
      </MaxWidthWrapper>
